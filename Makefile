...
AH_SRC_DB_DIR=$(CURDIR)/$(AH_DB_SUBDIR)
AH_BUILD_DB_DIR=$(AH_CURRENT_BUILD_PATH)/$(AH_DB_SUBDIR)
...
define CfgSquidGuard
        if [ ! -f $(AH_BUILD_DB_DIR)/Makefile ]; then \
				cd $(AH_BUILD_DB_DIR) && lndir -silent $(AH_SRC_DB_DIR); \
                cd $(AH_BUILD_DB_DIR)/build_unix; \
                ../dist/configure \
...
ifneq ($(DEPDIRS),)
	...
	@$(InitAhInst)
	@$(AH_MKDIR) $(AH_BUILD_DB_DIR)
	#$(AH_CP) -rf $(AH_DB_SUBDIR)/* $(AH_BUILD_DB_DIR)/
	...
	$(CfgSquidGuard)
	$(AH_MAKE) -C $(AH_BUILD_DB_DIR)/build_unix
	$(AH_MAKE) -C $(AH_BUILD_DB_DIR)/build_unix DESTDIR=$(AH_ROOTFS_DIR)/berkeley install
	#$(AH_MKDIR) $(AH_ROOTFS_DIR)/opt/ah/bin
	#$(AH_MV) $(AH_ROOTFS_DIR)/squid/usr/local/squid/sbin/squid $(AH_ROOTFS_DIR)/opt/ah/bin/ah_squid
	$(AH_CP) -rf $(AH_ROOTFS_DIR)/berkeley/usr/local/BerkeleyDB.4.6 $(AH_ROOTFS_DIR)/usr/local/
	$(AH_RMDIR) $(AH_ROOTFS_DIR)/usr/local/BerkeleyDB.4.6/bin
	$(AH_RMDIR) $(AH_ROOTFS_DIR)/usr/local/BerkeleyDB.4.6/docs
	#$(AH_MV) $(AH_ROOTFS_DIR)/nginx/var/log/ $(AH_ROOTFS_DIR)/var/log/nginx/
	#$(AH_TARGET_STRIP) $(AH_ROOTFS_DIR)/opt/ah/bin/ah_squid
	$(AH_RMDIR) $(AH_ROOTFS_DIR)/berkeley
lib:
...
install:
	...
	#$(AH_MKDIR) $(AH_ROOTFS_DIR)/opt/ah/bin
	#$(AH_MV) $(AH_ROOTFS_DIR)/squid/usr/local/squid/sbin/squid $(AH_ROOTFS_DIR)/opt/ah/bin/ah_squid
	#$(AH_CP) -rf $(AH_ROOTFS_DIR)/squid/usr/local/squid/ $(AH_ROOTFS_DIR)/usr/local/
	#$(AH_MV) $(AH_ROOTFS_DIR)/nginx/var/log/ $(AH_ROOTFS_DIR)/var/log/nginx/
	#$(AH_MV) $(AH_ROOTFS_DIR)/nginx/var/log/ $(AH_ROOTFS_DIR)/var/log/nginx/
	#$(AH_TARGET_STRIP) $(AH_ROOTFS_DIR)/opt/ah/bin/ah_squid
...
clean:
	if [ -f $(AH_BUILD_DB_DIR)/Makefile ]; then \
		$(AH_MAKE) -C $(AH_BUILD_DB_DIR) clean; \
	fi;
instclean:
	@$(CleanAhInst)
